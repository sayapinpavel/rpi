...
gst-launch-1.0 v4l2src device=/dev/video0 ! queue ! video/x-raw,width=${WIDTH},height=${HEIGHT},framerate=25/1 ! videoconvert ! queue ! x264enc tune=zerolatency ! queue ! udpsink host=${IP_ADDRESS} port=${PORT}